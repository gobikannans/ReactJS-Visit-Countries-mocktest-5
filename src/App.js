...
class App extends Component {
  ...
  onClickVisit = activeId => {
    const {listOfCountries} = this.state
    const visitedCountry = listOfCountries.map(eachItem => {
    ...
    this.setState({listOfCountries: visitedCountry})
  }

  onClickRemove = activeId => {
    const {listOfCountries} = this.state
    const visitedCountry = listOfCountries.map(eachItem => {
      if (eachItem.id === activeId) {
        const removedListItem = {
          ...eachItem,
          isVisited: !eachItem.isVisited,
        }
  ...
  renderVisitedCountriesList = () => {
    ...
    const emptyCountryList = updatedCountryList.length > 0
    return (
      <>
        {emptyCountryList ? (
          <ul className="visited-list-countries">
